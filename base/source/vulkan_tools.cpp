...
void SetImageLayout(
  ...
  switch (old_image_layout) {
    ...
    default: {
      std::cerr << "Unsupported src layout" << std::endl; 
    }
  }
  ...
  switch (new_image_layout) {
    ...
    default: {
      std::cerr << "Unsupported dst layout" << std::endl; 
    }
  }
...
void SetImageLayoutAndExecuteBarrier(
  ...
  VkCommandBufferBeginInfo cmd_buff_begin_info = tools::inits::CommandBufferBeginInfo();
  ...
  VK_CHECK_RESULT(vkWaitForFences(device.device(), 1U,
                  &copy_fence, VK_TRUE, UINT64_MAX)); 
...
VkFenceCreateInfo FenceCreateInfo(VkFenceCreateFlags flags) {
  ...
  structure.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
  structure.pNext = nullptr;
  structure.flags = flags;
...
VkDescriptorPoolCreateInfo DescriptrorPoolCreateInfo(
    uint32_t                       maxSets,
...
VkSubpassDescription SubpassDescription(
  ...
  VkSubpassDescription structure = {
    0U,
    pipelineBindPoint,
...
VkSpecializationMapEntry SpecializationMapEntry(
    ...
    uint32_t    offset,
    size_t      size) {
  VkSpecializationMapEntry structure = {
    constantID,
    offset,
    size
  };
