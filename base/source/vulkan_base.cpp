...
static const int32_t kWindowWidth  = 800;
static const int32_t kWindowHeight = 600;

#ifndef NDEBUG
static const std::vector<const char*> kInstanceDebugExtensions = {
  VK_EXT_DEBUG_REPORT_EXTENSION_NAME
};

static const std::vector<const char*> kInstanceDebugValidationLayers = {
  ...
  if (device_.IsDeviceVaild()) {
    ...
    if (copy_cmd_buff_ != VK_NULL_HANDLE) {
      vkFreeCommandBuffers(
          device_.device(),
          device_.graphics_queue().cmd_pool,
          1U,
          &copy_cmd_buff_);
      copy_cmd_buff_ = VK_NULL_HANDLE;
    }
    ...
    if (post_present_cmd_buffers_.size() > 0) {
      vkFreeCommandBuffers(
          device_.device(),
          device_.present_queue().cmd_pool,
    ...
    if (image_available_semaphore_ != VK_NULL_HANDLE) {
      vkDestroySemaphore(device_.device(), image_available_semaphore_,
                         nullptr);
      image_available_semaphore_ = VK_NULL_HANDLE;
    }
...
void VulkanBase::CreateSurface(GLFWwindow *window) {
  if (glfwCreateWindowSurface(instance_, window, nullptr,
                              &surface_) != VK_SUCCESS) {
...
void VulkanBase::CreateSwapChain(const uint32_t width, const uint32_t height) {
  swapchain_.InitAndCreate(device_.physical_device(),
...
void VulkanBase::ResetGraphicsCmdBbuffer() {
  ...
  for (uint32_t i = 0U; i < num_cmd_buffs; i++) {
    LOG("Cmd buff num " << i << ": " <<
        graphics_queue_cmd_buffers_[i]);
    VK_CHECK_RESULT(vkResetCommandBuffer(
        graphics_queue_cmd_buffers_[i],
