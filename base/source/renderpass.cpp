...
Renderpass::Renderpass(const eastl::string &name)
    : attachments_(),
      ...
      is_vkrenderpass_created_(false),
      name_(name),
      device_(nullptr),
...
void Renderpass::AddSubpassColourAttachmentRef(
    uint32_t subpass_id,
    uint32_t attach_id,
...
void Renderpass::AddSubpassDepthAttachmentRef(
    uint32_t subpass_id,
    uint32_t attach_id,
    VkImageLayout layout) {
  VKS_ASSERT(subpass_id <= (SCAST_U32(subpasses_.size()) - 1U),
...
VkRenderPass Renderpass::CreateVulkanRenderpass(const VulkanDevice &device) {
  ...
  VkRenderPassCreateInfo render_pass_create_info =
    tools::inits::RenderPassCreateInfo(
      SCAST_U32(attachments_.size()),
      attachments_.data(),
      SCAST_U32(subpass_descs.size()),
      subpass_descs.data(),
      SCAST_U32(dependencies_.size()),
      dependencies_.data());
...
void Renderpass::BeginRenderpass(
  ...
  vkCmdBeginRenderPass(cmd_buff, &render_pass_begin_info,
                       subpass_contents);
